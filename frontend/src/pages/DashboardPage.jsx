...
export default function DashboardPage() {
  ...
  const pagar = async (planId) => {
    setPagoMsg('Redirigiendo a MercadoPago...');
    try {
      ...
      setTimeout(() => { window.location.href = data.payment_url; }, data.modo_prueba ? 1200 : 0);
    } catch (e) {
    ...
    <div style={s.root}>
      ...
      <aside style={s.sidebar}>
        <div style={s.sidebarTop}>
          <div style={s.logo}>
            <span style={{ color: '#7c6af7', fontSize: '20px' }}>⬡</span>
          ...
          {loading ? (
            ...
            >
              <div style={s.noteCardHeader}>
                ...
                {nota.is_pinned ? <span style={{ color: '#7c6af7', fontSize: '12px' }}>📌</span> : null}
              </div>
            ...
            {selectedNota ? (
              <div style={s.editor}>
                <div style={s.editorToolbar}>
                  {editMode ? (
                    <>
                      <button onClick={guardar} disabled={saving} style={{ ...s.toolBtn, background: '#7c6af7', color: '#fff' }}>
        ...
        {view === 'stats' && stats && (
          <div style={s.statsView}>
            ...
            <div style={s.statsGrid}>
              ...
              <div style={s.statCard}>
                <div style={{ ...s.statNum, color: '#7c6af7' }}>{stats.plan}</div>
                ...
                <div key={plan.id} style={{ ...s.planCard, ...(plan.nombre === 'Pro' ? s.planCardFeatured : {}) }}>
                  ...
                  {plan.precio > 0 && (
                    user?.plan?.toLowerCase() === plan.nombre.toLowerCase() ? (
                      ...
                      <button onClick={() => pagar(plan.id)} style={{ ...s.payBtn, background: '#009ee3', marginTop: '16px' }}>
...
const s = {
  ...
  userPlan: { fontSize: '11px', color: '#7c6af7', textTransform: 'capitalize', fontWeight: '600' },
  btnNueva: { background: '#7c6af7', color: '#fff', border: 'none', borderRadius: '8px', padding: '9px 14px', cursor: 'pointer', fontSize: '14px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', width: '100%' },
  ...
  tag: { background: '#1e1a33', color: '#7c6af7', fontSize: '11px', padding: '2px 7px', borderRadius: '4px' },
  ...
  planBadge: { position: 'absolute', top: '-10px', left: '50%', transform: 'translateX(-50%)', background: '#7c6af7', color: '#fff', fontSize: '11px', fontWeight: '700', padding: '3px 12px', borderRadius: '20px' },
  planName: { fontSize: '18px', fontWeight: '700', color: '#f0f0f2', marginBottom: '6px' },
  planPrice: { fontSize: '22px', fontWeight: '700', color: '#7c6af7', marginBottom: '8px' },
